...
while True:
    atleta = input(
        'Digite nome do Atleta ou S para SAIR: '
        )
    if atleta == "S" or atleta == "s":
        break
    else:
        for i in range(1,6):
            salto.append(float(input(f'Digite o {i}º salto: ')))
print("O maior SALTO foi ", max(salto))
print("O menor SALTO foi ", min(salto))
print('A media dos saltos foi de:', (sum(salto) - max(salto) - min(salto))/3)
